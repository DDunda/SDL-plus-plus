...
namespace SDL
{
	...
	struct AudioDevice
	{
		...
		static void DontDestroyAudioDevice(DeviceID* ID);

		// This creates a smart pointer to a DeviceID with a custom destructor
		static std::shared_ptr<DeviceID> MakeSharedPtr(DeviceID ID);

		// This creates a AudioDevice from a Device ID, taking ownership of the ID
		static AudioDevice FromID(DeviceID ID);

		// This creates a AudioDevice from a Device ID, but does not take ownership of the ID
		static AudioDevice FromUnownedID(DeviceID ID);

		std::shared_ptr<DeviceID> ID = nullptr;

		int error = 0;

		AudioDevice(std::shared_ptr<DeviceID> ID);
		/**
	...
	struct WAV
	{
		...
		// This is custom destructor for smart pointers that destroys the WAVData through SDL
		static void DestroyWAV(WAVData* data);

		// This is custom destructor for smart pointers that does not destroy the WAVData. This is for pointers you do not own
		static void DontDestroyWAV(WAVData* data);

		// This creates a smart pointer to a WAVData with a custom destructor
		static std::shared_ptr<WAVData> MakeSharedPtr(WAVData* data);

		// This creates a WAV from a WAVData pointer, taking ownership of the pointer
		static WAV FromPtr(WAVData* data);

		// This creates a WAV from a WAVData pointer, but does not take ownership of the pointer
		static WAV FromUnownedPtr(WAVData* data);
	...
	struct AudioStream
	{
		// This is custom destructor for smart pointers that destroys SDL_AudioStream through SDL
		static void DestroyAudioStream(SDL_AudioStream* stream);

		// This is custom destructor for smart pointers that does not destroy the AudioStream. This is for pointers you do not own
		static void DontDestroyAudioStream(SDL_AudioStream* stream);

		// This creates a smart pointer to an SDL_AudioStream with a custom destructor
		static std::shared_ptr<SDL_AudioStream> MakeSharedPtr(SDL_AudioStream* stream);

		// This creates an AudioStream from an SDL_AudioStream pointer, taking ownership of the pointer
		static AudioStream FromPtr(SDL_AudioStream* stream);

		// This creates an AudioStream from an SDL_AudioStream pointer, but does not take ownership of the pointer
		static AudioStream FromUnownedPtr(SDL_AudioStream* stream);

		std::shared_ptr<SDL_AudioStream> stream = nullptr;
		int error = 0;

		AudioStream(std::shared_ptr<SDL_AudioStream> stream);
