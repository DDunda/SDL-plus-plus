...
namespace SDL
{
	...
	namespace WindowShape
	{
		 ...
		 * \return the window created, or NULL if window creation failed.
		 */
		inline static Window CreateShapedWindow(const char* title, Rect shape, WindowFlags flags = WindowFlags::INPUT_GRABBED)
			{ return Window::FromPtr(SDL_CreateShapedWindow(title, (unsigned int)shape.x, (unsigned int)shape.y, (unsigned int)shape.w, (unsigned int)shape.h, static_cast<Uint32>(flags))); }
		 ...
		 */
		inline static bool IsShapedWindow(const Window& window)
			{ return SDL_IsShapedWindow(window.window.get()) == SDL_TRUE; }
...
#define SDL_SHAPEMODEALPHA(mode) (mode == ShapeModeDefault || mode == ShapeModeBinarizeAlpha || mode == ShapeModeReverseBinarizeAlpha)
		 ...
		 */
		inline int SetWindowShape(Window& window, Surface& shape, const Mode& shape_mode)
		 ...
		 */
		inline int GetShapedWindowMode(Window& window, Mode& shape_mode)
